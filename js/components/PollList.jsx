...
var PollList = function(props) {
  var allPolls = props.allPolls;
  var pollNodes = [];

  for (var key in allPolls) {
    pollNodes.push(
      <Col key={key} xs={12} sm={6} md={6} className=''>
        <PollItem  poll={allPolls[key]} handlePollSelect={props.handlePollSelect} />
      </Col>
    );
  }
  ...
  return (
    <div>
      <div className='poll-container-header'>
        <h2 className='display-inline'>{props.header || 'Listing of all polls'}</h2>
      </div>
      <br />
      <Grid>
